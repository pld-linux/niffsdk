...
NIFF SDK to darmowy, będący własnością publiczną, niezależny od
platformy zestaw programistyczny dla programistów korzystających z
plików w formacie NIFF (Notation Interchange File Format). Jest to
zestaw bibliotek i narzędzi obsługujących odczyt, zapis i poruszanie
się po plikach NIFF. Dołączone są także dokumentacja, przykładowy
kod i przykładowe pliki NIFF. NIFF SDK umożliwia programistom dodanie
możliwości odczytu i zapisu plików NIFF do istniejącego programu bez
pisania samemu obsługujących te operacje funkcji.

Ten pakiet zawiera biblioteki współdzielone NIFF.
...
Pliki nagłówkowe i dokumentacja programisty NIFF SDK.
